...
<?php if ($this->options->WebPjax): ?>
  ...
  <script>$(document).pjax('a[href^="<?php Helper::options()->siteUrl()?>"]:not(a[target="_blank"],a[no-pjax]), a[href^="?"], a[href^="/"]',{container:'#pjax-load',fragment:'#pjax-load',timeout:8000}).on('pjax:send',function(){NProgress.start()}).on('pjax:complete',function(){NProgress.done();if(typeof hljs !== 'undefined'){hljs.initHighlightingOnLoad();}if(typeof MathJax !== 'undefined'){MathJax.typeset();}if(typeof renderMathInElement !== 'undefined'){renderMathInElement(document.body,{delimiters:[{left:"$$",right:"$$",display:true},{left:"$",right:"$",display:false},{left:'\\(',right:'\\)',display:false},{left:'\\[',right:'\\]',display:true}]});}});$(document).on('pjax:success',function(){$.getScript('<?php $this->options->themeUrl('/assets/js/features.js'); ?>',function(){});$.getScript('<?php $this->options->themeUrl('/assets/js/codecopy.js'); ?>', function() {});$.getScript('<?php $this->options->themeUrl('/assets/js/toc.js'); ?>', function() {})});</script>
<?php endif; ?>
